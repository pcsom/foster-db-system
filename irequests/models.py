...
class itemsRequest(models.Model):
    ...
    #All basic info
    def __str__(self):
        return f"{self.getStatus()} {self.forUser}'s Request for {self.forChild} on {self.date.date().strftime('%m/%d/%Y')}"

    #Esxclude user's name
    def childDateInfo(self):
        return f"{self.getStatus()} Request for {self.forChild} on {self.date.date().strftime('%m/%d/%Y')}"
    ...
    def get_fields(self, strDate=False):
        ...
        if strDate:
            theDate = str(theDate)
        finalfields = {"name": self.__str__(), "childDateInfo": self.childDateInfo(), "receiving": self.preferredMethodOfReceiving, "date": theDate, "fields": []}
        parsedData = self.formData
        if isinstance(parsedData, str):
            parsedData = json.loads(parsedData)
        for field in parsedData:
            val = parsedData[field]
            if val and val != 'False':
                newVal = ": " + str(val)
                if newVal == ': True':
                    newVal = " "
                finalfields['fields'].append((processName(field), newVal))
